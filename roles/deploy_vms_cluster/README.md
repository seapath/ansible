...
|--------------------|----------|-------------|---------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| vm_disk            | No       | String      | vms_disks_directory + item + ".qcow2" | Path to the VM disk in `qcow2` format on the the Ansible machine. Mutually exclusive with `vm_remote_disk`.                                                                                                                             |
| vm_remote_disk     | No       | String      |                                       | URL (`http`/`https`/`ftp`/`ftps`) of the VM disk, fetched directly on the target. Requires `vm_disk_checksum`. Mutually exclusive with `vm_disk`.                                                                                       |
| vm_disk_checksum   | No       | Dict        |                                       | Dict `{<algo>: <hex>, ...}` to verify the system disk after staging. Required for `vm_remote_disk`, optional for `vm_disk`. May list multiple algorithms; the strongest available is used (preference: `sha512` > `sha384` > `sha256`). |
