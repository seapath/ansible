...
| vm_pinning_profile |               | String          |           | seapath-alloc CPU pinning profile (YAML). Stored as RBD image metadata `_seapath_alloc` at creation and propagated on clone, and applied by the libvirt hook at every VM start and migration. See the `deploy_seapath_alloc` role. |
