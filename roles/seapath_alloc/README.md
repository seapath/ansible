...
**From the inventory (recommended).** Set `seapath_alloc` in the VM's
...
so the same `seapath_alloc` variable is written by `deploy_vms_standalone`
...
Import `roles/seapath_alloc/files/grafana-seapath-alloc.json` into
