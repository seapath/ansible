...
| cephadm_prometheus_listen_address   | No     | String  | see defaults | Administration IP the mgr prometheus exporter binds to on each host. Defaults to `ip_addr`, same as `deploy_prometheus_exporters_listen_address`, and to `127.0.0.1` when `deploy_metrics_proxy_enabled` is set. Override per host in inventory if needed. |
...
- `ceph config set mgr.<hostname>.<id> mgr/prometheus/server_addr <admin-ip>` on each mgr
  ...
  metrics proxy is deployed with `deploy_metrics_proxy_enabled`)
...
the gathered hostname, then to `inventory_hostname`) and sets the listen address to its own
administration IP. The `ceph` commands are delegated to the first Ceph node. The address is
read in each host's own variable context, so an `ip_addr` built from other inventory
variables is resolved normally instead of being passed to `ceph` as a literal `{{ ... }}`
string.
