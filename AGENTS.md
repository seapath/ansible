...
- **Real-time tuning**: isolated CPUs (`isolcpus`), tuned profiles, NIC IRQ
  affinity pinning, dynamic per-VM CPU pinning (`seapath_alloc`).
...
- **Real time is the product.** Anything touching isolated CPUs, tuned, IRQ
  ...
  `roles/seapath_alloc/ARCHITECTURE.md` — read it before touching that
...
```
qa (ansible-lint) → unit (pytest + coverage) → molecule → debian / yocto / sles / oraclelinux / virtual-cluster integration
```

Integration tests run on self-hosted runners, driven by `.github/workflows/_tests.yml` against
private inventory repos (e.g. `insatomcat/seapath_inventories_ci`, `seapath-ci/ci-seapath-yocto-inventories`).
A CentOS path (`ci-centos.yml`) still clones the external `seapath/ci` repo, but it has been
unplugged from the main `ci.yml` orchestrator since there is no CentOS runner available; it can be
re-enabled once one exists.
