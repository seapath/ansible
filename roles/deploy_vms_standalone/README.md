...
guest belongs to which. Declare `standalone_VMs` and `cluster_VMs` as children
of `VMs` to say it:
...
| vm_pinning_profile |               | String          |           | seapath-alloc CPU pinning profile (YAML). Written to `/etc/seapath/alloc.d/<vm>.yaml` and applied by the libvirt hook at every VM start. Alternative to `cpuset` / `emulatorpin`: do not set both. See the `deploy_seapath_alloc` role. |
