...
| seapath_alloc |               | String          |           | seapath-alloc CPU pinning profile (YAML). Written to `/etc/seapath/alloc.d/<vm>.yaml` and applied by the libvirt hook at every VM start. Alternative to `cpuset` / `emulatorpin`: do not set both. See the `seapath_alloc` role. |
