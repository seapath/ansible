...
def test_create_carries_the_seapath_alloc_profile(run, vm_manager, image):
    profile = "vcpus:\n  isolation: exclusive_physical\n"

    run(command="create", name="vm0", xml="<domain/>", system_image=image,
        seapath_alloc=profile)

    assert only_call(vm_manager)[1][0]["seapath_alloc"] == profile


def test_clone_carries_the_seapath_alloc_profile(run, vm_manager):
    profile = "vcpus:\n  isolation: exclusive_logical\n"

    run(command="clone", name="vm1", src_name="vm0", seapath_alloc=profile)
...
def test_clone_without_a_profile_lets_vm_manager_copy_the_source_one(
    ...
    assert only_call(vm_manager)[1][0]["seapath_alloc"] is None
